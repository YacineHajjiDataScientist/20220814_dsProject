...
# 
# - **Session**  
# Correspond à l'import des modules, à la création des fonctions, à la définition des directory  
# - **Import**  
# Import des bases de données et informations sur la taille des bases de données  
# - **Data-Processing**  
# Création de variables dérivées, rafinement de variables existantes, modification des modalités, merge des dataframes et export, QC  
# - **Descriptive statistics**  
# Partie 1: Description des tables, complétion des variables, préparation à remplir le mapping de variables.  
# Partie 2: Graphiques univariés de toutes les variables sur lesquelles j'ai travaillé, en général j'ai fait pour chaque variable: 1 tableau de sample size par modalité, des proportions de la variable target normalisés par modalité et un heatmap pour lire plus rapidement l'info. On peut aussi retrouver quelques graphiques différents qui ont permis de se rendre compte qu'il serait trop difficile de travailler sur certaines variables, ou encore des graphiques de tentative de représentation des données.  
# Partie 3: Analyse inférentielle/statistique autour de la variable target, j'ai fait des graphiques qui correspondent à cette variable target, calculé les V de Cramer et p-valeurs entre chaque variable indépendante et la target.  
# Partie 4: Graphiques multivariés et matrice de V de Cramer pour analyser la relation entre les variables indépendantes (1 heatmap par table).  
# - **Pre-processing**  
# Sélection des variables explicatives pour la partie Machine Learning, en se basant sur plusieurs facteurs: complétion et nombre de modalités des variables, type de la variable, relation entre la variable (si deux variables liées, on garde celle qui semble la plus intéressantes en fonction de la variable target), et enfin suppression des 27 features les moins intéressantes, de manière empirique avec le feature importance du XGBoost.  
...
# ### Part 4- Relation between independent variables
...
# # --------------------------------------Pre-processing--------------------------------------
# ### Selection of variables based on NA prevalence, number of modalities, type, relation between variables and XGBoost feature importance
...
# $\color{#fc0303}{\text{
# This corresponds to exploration but has not been kept in the end.
# }}$
# $\color{#fc0303}{\text{
# The idea was to have 1 dataset by method problematic (e.g. one dataset for methods working with NAs and one dataset without.
# }}$
# $\color{#fc0303}{\text{
# But in the end we wanted to compare the different methods so we created one dataset which can work for all Machine Learning methods---
# }}$
...
# $\color{#fc0303}{\text{---end of exploration.}}$

# ### XGBoost (launched for feature importance selection)
...
# $\color{#fc0303}{\text{
# Exploration of methods performances---
# }}$
